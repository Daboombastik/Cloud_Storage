...
public class StoragePanelFxController implements Initializable {
	...
	private void copyFileFromCloud() {
		...
		FileOperationsMessage fileOperationsMessage = new FileOperationsMessage(FileOperationsMessage.FileOperation.COPY, focusedFileLine.getName());
	...
	private void moveFileFromClod() {
		FileParameters focusedFileLine = cloudTable.getSelectionModel().getSelectedItem();
		if(focusedFileLine == null) return;
		FileOperationsMessage fileOperationsMessage = new FileOperationsMessage(FileOperationsMessage.FileOperation.MOVE, focusedFileLine.getName());
		Network.sendMsg(fileOperationsMessage);
	}
	...
	private void deleteFileFromCloud() {
		FileParameters focusedFileLine = cloudTable.getSelectionModel().getSelectedItem();
		if(focusedFileLine == null) return;
		FileOperationsMessage fileOperationsMessage = new FileOperationsMessage(FileOperationsMessage.FileOperation.DELETE, focusedFileLine.getName());
		Network.sendMsg(fileOperationsMessage);
	}
